...
func transition_to(scene, transition, data: Dictionary = {}) -> void:
	var transition_scene = transition.instantiate() as GodotParadiseSceneTransition
...
func on_finished_transition(data: Dictionary, next_scene) -> void:
	...
	if _is_loading_screen(data):
		...
		push_error("GodotParadiseSingletonBundle | SceneTransitioner: The scene {scene} cannot be transitioned".format({"scene": scene}))
